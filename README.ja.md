...
- 再帰的な子ミッション（Child Mission）と、関連付けられた後続 Mission。
...
シナリオでは、共通の reviewer を持つ 2 つの software-development Mission、Worker が正式な
Work Proposal を通じて提案する下位の WorkItem、セキュリティの子ミッション、Worker 間の
clarification、分離された 2 つの execution slot、checkpoint のみで行われる preemption、
WorkItem のブロック／再開、Coordinator review、人間による 1 回の change request、正確に
