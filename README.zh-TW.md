...
- 遞迴的子任务（Child Mission，即獨立的 Mission，而非工作項）和相互關聯的後續 Mission；
...
包含由 Worker 透過正式 Work Proposal 提出的下層工作項、一個安全子任务、Worker 之間的
...
建立一次性的本機金鑰、一個由 Organization 簽名且包含 Agent Card 的 Agent Registry，以及完整的
Agent Registry 簽名金鑰 snapshot：
