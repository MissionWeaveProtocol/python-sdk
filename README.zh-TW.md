...
- 對等 Conversation，以及顯式的 Work Proposal、授權、發出任務、接受任務、所有權、
...
- 權威的六維 Mission/WorkItem 額度分配和累計使用量核算；
...
符合性測試命令會使用格式檢查，根據 21 個內建的 Draft 2020-12 schema 驗證全部 43
...
包含由 Worker 正式提出的子任務、一個子安全 Mission、Worker 之間的澄清交流、兩個隔離
的執行槽位、僅在 checkpoint 處發生的搶佔、任務受阻與復原、Coordinator 審查、一次人類
...
建立一次性的本機金鑰和由 Organization 簽名的 registry：
